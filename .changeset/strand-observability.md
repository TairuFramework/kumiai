...
`@kumiai/mls` persists received commits and ledger bootstrap before notifications, restoring the prior in-memory state if persistence fails.

`@kumiai/mls-rpc` persists recovery handles before adoption and uses the MLS handle persistence boundary for commits and ledger bootstrap.
