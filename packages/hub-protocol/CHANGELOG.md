...
- Constrain the `topicID` schema to `^[A-Za-z0-9_-]{43}$` — the 43-character base64url shape every
  topicID already has (`toB64U` of a 32-byte HKDF/SHA-256 output). (A safe syntactic superset: the
  canonical final-char subset is deliberately not pinned.) Applied to all six sites: the publish, subscribe,
