...
describe('createEncryptedHubTunnelTransport decrypt failures', () => {
  test('decrypt failure drops the frame, emits observability events, session continues', async () => {
    const hub = new FakeHub()
    const sessionID = 'session-decrypt-fail'
    const groupID = 'group-decrypt-fail'
    ...
    // WHY: pre-arming the receiver's encryptor before any send means the very first
    // inbound ciphertext deterministically fails to decrypt, while subsequent frames
    // succeed. Simpler than wrapping the encryptor with a one-shot interceptor.
