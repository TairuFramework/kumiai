 ...
 */
/** One buffered app frame. `sealed` goes `null` once done — delivered or dead. */
...
export function createGroupPeer<Protocols extends Record<string, ProtocolDefinition>>(
  ...
  /**
   * The current SEGMENT's retained app frames, per protocol, in log order. A segment is the run
   * of epochs between two roster changes — exactly the run one app topic spans — so it is pulled
   * once (below) and dispensed epoch by epoch as the commit walk passes through it.
   */
  let appSegment = new Map<string, Array<AppFrame>>()

  /**
   * The app lane's TWO read positions per protocol for the CURRENT segment — not the same
   * position, and must not be conflated:
   *
   * - `position` is the DURABLE CURSOR: the last frame this drain is done with, and the last
   *   thing {@link appCursorStore} was told. Held here so a drain that moved nothing writes
   *   nothing.
   * - `fetched` is the LAST-FETCHED POSITION: how far the buffer has been filled from. Runs AHEAD
   *   of the cursor whenever a buffered frame is not done, and is where the next pull resumes —
   *   so a re-pull costs one short page, not the whole tail.
   *
   ...
   *
   * Staged rather than written straight into {@link appSegment}: the push arrives on the mux's
   * own drain loop, outside every lane operation, and writing there would splice the array
   * {@link deliverAppFrames} is midway through iterating and awaiting inside. Everything that
   * touches the buffer goes through {@link runAppLane} instead; this is the hand-off.
   *
   ...
   *
   * NOT the commit mutex: that one resets the journal-replay flag on entry, so a cursor write
   * taking it would wrongly tell the next pull the journal had not been replayed. And it must not
   * be reentrant, since the app lane is reached both from inside a commit walk and from the mux's
   * push loop — the buffer is an ordered array {@link deliverAppFrames} iterates and awaits
   * inside, and a push splicing it mid-iteration would step over frames.
   */
  ...
  const captureAnchor = async (): Promise<void> => {
    ...
    // belongs to. Dropping UNDELIVERED frames is not a loss: the walk already read everything
    // openable on the way here, so what remains is unopenable forever. Cursors go with the buffer;
    // nothing is cleared at the STORE, since a cursor is keyed by topic and stays true of the
    // topic being left.
    appSegment = new Map()
    appCursors = new Map()
   ...
   *
   * A POSITION IS TAKEN ONCE: the live lane is pushed a frame and the pull reads it back out of
   * the log, so a second entry for a position already held is a second delivery of one message —
   * the duplicate the cursor exists to make impossible. A repeat is a reconcile, never an append,
   * and only ever marks a frame DONE, never undoes it: `sealed: null` says the live lane had this
   * frame at its seal epoch, and a pull reading it back afterwards must not redeliver it.
   *
   * Kept in log order by insertion, not append, since the two deliverers do not arrive in one
   * order: a pull runs from behind the live stream and returns frames the pushes already brought.
   */
   ...
   *
   * The advance stops at the FIRST frame that is not done — the safety property: a cursor may
   * only pass a frame that is DELIVERED or DEAD. A done frame further along is left in place,
   * since a position is a place in the LOG and passing it passes everything before it too.
   *
   ...
   *
   * A MAILBOX frame is skipped outright — nothing to advance over. The class travels with the
   * push because ephemeral and logged app traffic share one topic, and guessing from the topic
   * would move the cursor over frames the log does not contain.
   */
   ...
   * live traffic alone — a peer that is online and walks no commit still has to keep its
   * position, or a group quiet on the commit lane and busy on the app lane re-delivers its whole
   * backlog on the next restart.
   *
   ...
   * anything still holding bytes waits for {@link deliverAppFrames}, which runs where the handle
   * is held still.
   *
   ...
   * key material, and those bytes are ciphertext forever. Per-FRAME-EPOCH, not per-rotation: every
   * epoch inside a segment is dispensed as the walk passes through it.
   *
   ...
   * {@link justifiedEpochCeiling} for the same reason — unbounded, it would pin the cursor behind
   * it forever.
   *
   * The buffer is walked whole, not stopped at the first frame that is not this epoch's, since the
   * front can still hold a frame from an epoch the handle already passed (a journal replay
   ...
   * it is not — see {@link advanceAppCursor} for the rule.
   *
   * A FAILED PULL STALLS THE WALK: `loadAppSegment` raising propagates and no epoch is passed
   ...
   * ({@link scheduleAppLaneSync}), which runs off the mux's push loop under no lock at all. Two
   ...
   * {@link advanceAppCursor}'s read of a done run and its splice silently drops frames.
   *
   * NO DEADLOCK against the commit mutex: every call here is from inside `runSerial`, taking the
   * app lane second, and nothing inside it takes `runSerial` back — the one path that could, a
   * host handler re-entering from the delivery below, already deadlocks on `runSerial` itself.
   */
