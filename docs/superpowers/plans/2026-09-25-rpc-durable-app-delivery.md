...
### 2026-09-25 — Second blind whole-branch review

- **Findings:** F1–F6 were confirmed. During a roster rotation the handle could advance while anchor export was pending, letting a new-epoch app frame land on the old topic. A worker timer could fire before its producing commit operation released. Stall deduplication used one key for all protocols. The pending conformance clauses omitted exact sender identity and wrong-AAD retry. The fake epoch export was reversible, and the decrypt comment contradicted the bounded past-epoch window.
- **Changed:** A seal barrier covers handle advance through anchor assignment, including an export pause; the existing anchor identity check still handles a completed rotation during wrap. A failed advance after the epoch changes blocks further sealing until a successful anchor capture. Worker starts use the peer's post-operation outbox. Stall keys are tracked per protocol. Pending conformance checks exact sender DID and wrong-AAD rejection before persistence. Fake exports derive epoch and label keys with HMAC. The reset and decrypt comments and MLS-RPC README now describe the implemented behavior.
- **Accepted and why:** The old-topic new-epoch frame path is closed for conforming local publishers, so reset retains its segment-boundary behavior. The separately pending rule for app frames claiming an epoch above the justified ceiling was left unchanged. The seal barrier resolves at anchor assignment, before anchor-store I/O, so an existing reentrant dispatch from anchor save remains possible.
- **Tests and mutations:** F1, F2, and F4 tests failed against their original behavior. Removing each fix made its test fail again. A failed-export dispatch test also failed before its guard and when that guard was removed. Replacing HMAC with the old XOR made the new secret test fail. Wrong-AAD bypasses in both real and fake ports, and a wrong persisted sender DID in the real port, failed the new conformance assertions; all mutations were restored. The rotation file and both new timing cases passed 20 consecutive runs each.
- **Verify:** `rtk proxy pnpm turbo run test:types test:unit --force --concurrency=2`: 49 successful, 0 cached. RPC conformance against real and fake: 51 tests each. Hub server, tunnel, and wake conformance: 54, 4, and 16 tests. Integration: 8 files, 43 tests. `rtk proxy pnpm run lint`: clean.

### 2026-09-25 — Future-epoch retention decision

- **Premise:** Verified in source and with red tests. The app drain previously used a ceiling computed from commit frames returned by the hub. When the hub omitted the commit producing epoch E+1, the drain marked an honest E+1 frame done and persisted a cursor past it. A forged far-future frame without a justifying commit produced no stall notice.
- **Decision:** Retain every frame claiming an epoch above this peer's current epoch until the peer reaches that epoch or an operator calls `dropAppFrame`. A hub omission must not silently lose the frame. A forged claim can hold delivery until the operator drop; one `future-epoch` stall notice surfaces the wait. Frames below the current epoch remain dead. The commit-derived ceiling and its dedicated pager/test were removed because nothing else used them.
- **Tests and mutations:** The new omitted-commit and forged-claim tests failed before the change, then passed. They prove that the cursor stops before the frame, the frame opens after the commit reappears, one notice is raised, and an operator drop survives restart. The new test file passed 20 consecutive runs. Mutating the future-epoch retention branch, the notice deduplication guard, and the operator drop each failed its focused test; source was restored after each.
- **Verify:** Forced `rtk proxy pnpm turbo run test:types test:unit --force --concurrency=2` tail: `Tasks: 49 successful, 49 total`; `Cached: 0 cached, 49 total`. RPC fake port and hub conformance: 2 files, 78 tests; MLS-RPC real port: 1 file, 51 tests; hub server: 2 files, 54 tests; hub tunnel: 1 file, 4 tests; hub wake: 1 file, 16 tests. Sequential integration: 8 files, 43 tests. Final `rtk proxy pnpm run lint`: 387 files checked, no fixes applied.
